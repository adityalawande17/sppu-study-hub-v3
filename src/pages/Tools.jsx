...
const tools = [
        ...
        <div className="tool-body">
          ...
          {val && (
            <div className="tool-result">
              <div>
                <div className="tool-result-label">Percentage</div>
        ...
        <div className="tool-body">
          ...
          {res && (
            <div className="tool-result" style={{ flexWrap: "wrap", gap: 10 }}>
              <div>
                <div className="tool-result-label">Grade</div>
                <div className="tool-result-value">{res.grade}</div>
              </div>
              <div>
                <div className="tool-result-label">Grade Point</div>
                <div className="tool-result-value">{res.gp}</div>
              </div>
              <div>
                <div className="tool-result-label">Category</div>
                <div
                  style={{
                    fontSize: 14,
                    fontWeight: 600,
    ...
    Component: function AttendCalc() {
      ...
      if (total && attended) {
        ...
        if (!isNaN(t) && !isNaN(a) && t > 0 && a >= 0 && a <= t) {
          ...
          const needMore =
            a < Math.ceil(0.75 * t) ? Math.ceil(0.75 * t) - a : 0;
      ...
      return (
        <div className="tool-body">
          ...
          {result && (
            <div className="tool-result" style={{ flexWrap: "wrap", gap: 10 }}>
              <div>
                <div className="tool-result-label">Attendance</div>
                <div
                  className="tool-result-value"
                  style={{ color: result.status ? "#16a34a" : "#dc2626" }}
                >
                  {result.pct}%
                </div>
              </div>
              {result.status ? (
                <div>
                  <div className="tool-result-label">Can Miss</div>
                  <div
                    className="tool-result-value"
                    style={{ color: "#16a34a" }}
                  >
                    {result.canMiss < 0 ? 0 : result.canMiss} lectures
                  </div>
                </div>
              ) : (
                <div>
                  <div className="tool-result-label">Need More</div>
                  <div
                    className="tool-result-value"
                    style={{ color: "#dc2626" }}
                  >
                    {result.needMore} lectures
                  </div>
                </div>
              )}
            </div>
  ...
  },
  {
    ...
    Component: function GPACalc() {
      ...
      function addRow() {
        setRows((r) => [...r, { name: "", credits: "", marks: "" }]);
      }
      ...
      const valid = rows.filter(
        (r) => r.credits && r.marks && !isNaN(r.credits) && !isNaN(r.marks),
      );
      if (valid.length > 0) {
        const totalCredits = valid.reduce((s, r) => s + parseInt(r.credits), 0);
        const weightedGP = valid.reduce((s, r) => {
          const gp = gradePoint(r.marks);
          return s + (gp ? gp.gp * parseInt(r.credits) : 0);
        }, 0);
      ...
      return (
        <div className="tool-body">
          <div style={{ display: "grid", gap: 6, marginBottom: 10 }}>
            ...
            >
              {["Subject", "Credits", "Marks /100", ""].map((h) => (
                <div
                  key={h}
                  style={{
                    fontSize: 11,
          ...
          <button
            ...
            style={{ fontSize: 13, padding: "7px 12px", marginBottom: 10 }}
          >
            + Add Subject
          </button>
          {sgpa && (
            <div className="tool-result">
              <div>
                <div className="tool-result-label">Your SGPA</div>
                <div className="tool-result-value">{sgpa} / 10</div>
              </div>
              <div>
                <div className="tool-result-label">Percentage</div>
                <div
                ...
                >
                  {(parseFloat(sgpa) * 10 - 7.5).toFixed(2)}%
                </div>
              </div>
            </div>
          )}
...
export default function Tools() {
  useSEO({
    title:
      "SPPU Tools — SGPA to Percentage, CGPA to Percentage, Attendance Calculator | SPPU StudyHub",
    description:
      "Free SPPU calculators for engineering students. Convert SGPA and CGPA to percentage, calculate attendance, check ATKT eligibility, compute semester GPA.",
  });
    ...
    <div className="page-wrap">
      ...
      >
        ...
        <span className="section-sub">
          Calculators for SPPU engineering students
        </span>
      </div>

      <div className="info-strip" style={{ marginBottom: 24 }}>
        All calculations follow official SPPU formulas and grading schemes. The
        SGPA and CGPA to percentage formula is: Percentage = (GPA × 10) − 7.5,
        as specified by SPPU.
      </div>
