...
function ResourceRow({ resource }) {
  ...
  return (
    <div
      style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', border: '1px solid var(--border)', borderRadius: 8, background: 'var(--surface2)', transition: 'all .15s' }}
      onMouseEnter={e => { e.currentTarget.style.borderColor = isVideo ? '#dc2626' : 'var(--gold-dim)'; e.currentTarget.style.background = isVideo ? '#fff5f5' : 'var(--gold-pale)' }}
      onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.background = 'var(--surface2)' }}>

      {/* Icon */}
      <div style={{ width: 32, height: 32, borderRadius: 6, background: cfg.bg, color: cfg.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {resource.meta && (
          <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 1 }}>{resource.meta}</div>
        )}
      </div>
      ...
      <a
        ...
        onMouseEnter={e => { e.currentTarget.style.background = isVideo ? '#dc2626' : '#0a1628'; e.currentTarget.style.color = '#fff' }}
        onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = isVideo ? '#dc2626' : 'var(--heading)' }}>
      ...
      {practicals.map((p, i) => {
        ...
        return (
          <div key={i} style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
            ...
            <button
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
                <span style={{
                  ...
                  background: isOpen ? 'rgba(255,255,255,.18)' : 'var(--border)',
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  {p.desc && !isOpen && (
                    <div style={{ fontSize: 11, color: isOpen ? 'rgba(255,255,255,.55)' : 'var(--text-3)', marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.desc}</div>
                  )}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                {resCount > 0 && (
                  <span style={{ fontSize: 11, opacity: .75 }}>{resCount} resource{resCount !== 1 ? 's' : ''}</span>
                )}
                {resCount === 0 && (
                  <span style={{ fontSize: 11, opacity: .5 }}>No resources yet</span>
                )}
                <div style={{
                  width: 12, height: 12,
            ...
            {/* Expanded content */}
            {isOpen && (
              <div style={{ padding: '12px 14px', background: 'var(--surface)', borderTop: '1px solid var(--border)', display: 'grid', gap: 8 }}>
                {/* Description shown when open */}
                {p.desc && (
                  <p style={{ fontSize: 13, color: 'var(--text-3)', lineHeight: 1.5, paddingBottom: 4, borderBottom: '1px solid var(--border)' }}>
                    {p.desc}
                  </p>
                )}
                ...
                {resCount > 0 ? (
                  <div style={{ display: 'grid', gap: 6 }}>
                    {p.resources.map((r, j) => <ResourceRow key={j} resource={r} />)}
                  </div>
                ) : (
                  <div style={{ padding: '10px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 6, background: 'var(--surface3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--text-4)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                    </div>
                    <p style={{ fontSize: 13, color: 'var(--text-3)', lineHeight: 1.5 }}>
                      No resources uploaded yet for this practical. Add links, PDFs or YouTube videos in the subject JSON file.
                    </p>
                  </div>
                )}
              </div>
